...
namespace Microsoft.Azure.Management.Support
{
    ...
    public partial class MicrosoftSupportClient : ServiceClient<MicrosoftSupportClient>, IMicrosoftSupportClient, IAzureClient
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<AzureOperationResponse<CommunicationDetails>> AddCommunicationToSubscriptionTicketWithHttpMessagesAsync(string supportTicketName, AddCommunication addCommunication, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (supportTicketName == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "supportTicketName");
            }
            if (addCommunication == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "addCommunication");
            }
            if (addCommunication != null)
            {
                addCommunication.Validate();
            }
            if (SubscriptionId == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "this.SubscriptionId");
            ...
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("addCommunication", addCommunication);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "AddCommunicationToSubscriptionTicket", tracingParameters);
            }
            // Construct URL
            var _baseUrl = BaseUri.AbsoluteUri;
            var _url = new System.Uri(new System.Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "subscriptions/{subscriptionId}/providers/Microsoft.Support/supportTickets/{supportTicketName}/addCommunication").ToString();
            ...
            if(addCommunication != null)
            {
                _requestContent = SafeJsonConvert.SerializeObject(addCommunication, SerializationSettings);
                _httpRequest.Content = new StringContent(_requestContent, System.Text.Encoding.UTF8);
                _httpRequest.Content.Headers.ContentType =System.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json; charset=utf-8");
            }
            // Set Credentials
